...
データを、Bronze、Silver、Goldの３層の論理レイヤーで管理する手法である。
...
実装方法の詳細は公開されていないため、実際の設計指針については自分たちで検討する必要あり。
３層の論理レイヤーでは管理を上位のレイヤーとして捉えて、詳細レイヤーを設計することがおすすめ。
...
本資料は、Databricks におけるデータエンジニアリングの実装方法を、**命令型** と **宣言型** という観点から整理するための資料である。
目的は「どちらが優れているか」を決めることではなく、**それぞれが何を自動化し、何を開発者に委ねるのか** を理解することにある。
実務では Notebook、Lakeflow Spark Declarative Pipelines、dbt を単独で使うより、組み合わせて使うことが多い。

### 1. 結論

Databricks における代表的な実装スタイルを大まかに分類すると、次のようになる。ご提示の比較表でも、Notebook は命令型、SDP と dbt は宣言型として整理されている。Databricks 公式でも、宣言型の代表として Lakeflow Spark Declarative Pipelines が位置づけられている。 
...
命令型とは、**処理の手順を開発者が細かく記述する** スタイルである。

たとえば Notebook + PySpark では、次のようなことを自分で書く。
...
つまり、命令型では **「何をしたいか」だけでなく「どう実行するか」まで自分で持つ**。
その分、細かい最適化や例外処理を作り込みやすい。 ([Zenn][1])

#### 宣言型とは何か

宣言型とは、**最終的にどういうデータを得たいかを記述し、処理順序や実行計画の多くをフレームワーク側に委ねる** スタイルである。

Databricks の Lakeflow Spark Declarative Pipelines では、開発者は主に次を定義する。
...
すると、フレームワーク側が依存関係を解決し、実行順序や並列性、失敗時の再試行などを自動化する。Databricks 公式では、SDP の利点として automatic orchestration、declarative processing、incremental processing を挙げている。 ([Microsoft Learn][2])

#### 一番大事な違いは「誰が責任を持つか」

命令型と宣言型の差を、実務では次のように捉えると分かりやすい。
...
Databricks 公式は、SDP の利点として次を強調している。 ([SDP の利点は何ですか?](https://learn.microsoft.com/ja-jp/azure/databricks/ldp/concepts#what-are-the-benefits-of-sdp))

* **Automatic orchestration**
  実行順序や並列性を自動化しやすい

* **Declarative processing**
  数百行の手続きコードを、少ない宣言で置き換えやすい
...
* 依存関係が自然に表現される
* パイプライン全体を統一的に扱いやすい
* ただし、利用できる機能やオブジェクトに制約がある

### 4. まとめ

Databricks におけるデータエンジニアリングでは、命令型と宣言型は対立概念というより、**責任分担の違い** と捉えるのがよい。

* 命令型は柔軟だが、設計責任が重い
* 宣言型は制約があるが、運用を標準化しやすい
* 実務では両者を組み合わせるのが普通
* 技術選定では「書きやすさ」より「運用し続けやすさ」を見るべき
